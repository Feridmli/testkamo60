...
/**
 * CLEAN ORDER (CRITICAL FIX)
 * Baza JSONB-dən gələn məlumatları Seaport-un tələb etdiyi dəqiq formata salır.
 * Seaport 1.6 uyğunluğu üçün 'zoneHash' və 'totalOriginalConsiderationItems' vacibdir.
   */
...
token: item.token,
identifierOrCriteria: toStr(item.identifierOrCriteria |
| item.identifier |
| "0"),
...
| ZERO_BYTES32, // Seaport 1.6 üçün kritik
...
// Seaport yalnız düzgün şəbəkədə və konkret kontraktla yaradılır
...
if (isListed) {
if (isMine) {
actionsHTML = <div class="price-val">${displayPrice}</div> <button class="action-btn btn-list cancel-btn" disabled>Satışdadır</button>;
} else {
actionsHTML = <div class="price-val">${displayPrice}</div> <button class="action-btn btn-buy buy-btn">Satın Al</button>;
}
} else {
if (isMine) {
actionsHTML = <input type="number" placeholder="Price" class="mini-input price-input" step="0.001"> <button class="action-btn btn-list list-btn">Listələ</button>;
} else {
actionsHTML = <span style="font-size:12px; color:#666;">Sahibi: ${currentOwner.slice(0,6)}...</span>;
}
}
card.innerHTML = ${checkboxHTML} <div class="card-image-wrapper"> <img src="${image}" loading="lazy" decoding="async" onerror="this.src='[https://i.postimg.cc/Hng3NRg7/Steptract-Logo.png](https://i.postimg.cc/Hng3NRg7/Steptract-Logo.png)'"> </div> <div class="card-content"> <div class="card-title">${name}</div> <div class="card-actions"> ${actionsHTML} </div> </div>;
...
if(!val |
| isNaN(val) |
| parseFloat(val) <= 0) return notify("Düzgün qiymət yazın!");
...
if (!priceVal |
| isNaN(priceVal) |
| parseFloat(priceVal) <= 0) return alert("Toplu satış üçün düzgün qiymət yazın.");
...
// ConduitKey ZERO_BYTES32 olduğu üçün birbaşa Seaport-a approve veririk
...
return {
// Opensea komissiyasından qaçmaq üçün ConduitKey 0x00...
...
const offerItem = order.parameters.offer;
...
alert("Satış xətası: " + (err.message |
| err));
}
}
// ==========================================
// 7. BUY FUNCTION (DÜZƏLDİLMİŞ: MSG.VALUE ƏLAVƏ EDİLDİ)
// ==========================================
...
// ============================================================
// FIX: NATIVE TOKEN (APE) DƏYƏRİNİ HESABLAMAQ
// Seaport native token ilə alış-veriş edərkən avtomatik msg.value
// əlavə etmir, bunu biz etməliyik.
// ============================================================
let totalEthValue = ethers.BigNumber.from(0);
cleanOrd.parameters.consideration.forEach(item => {
// itemType 0 = NATIVE (ETH/APE)
// itemType 1 = ERC20
if (item.itemType === 0) {
totalEthValue = totalEthValue.add(ethers.BigNumber.from(item.startAmount));
}
});
console.log("Göndəriləcək Dəyər (Wei):", totalEthValue.toString());
// 3. Fulfill Order (VALUE OVERRIDE İLƏ)
...
overrides: {
value: totalEthValue.toString() // <--- ƏSAS DÜZƏLİŞ
}
});
